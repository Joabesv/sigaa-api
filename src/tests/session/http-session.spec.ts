...
test.only('if Sigaa http returns a page', async () => {
...
test.only('if Sigaa http send cookies', async () => {
  const http = createHTTPInstance();
...
test.only('if Sigaa http requests again if cookies change', async () => {
  const http = createHTTPInstance();
...
test.only('if Sigaa http requests again if noCache is enable', async () => {
  const http = createHTTPInstance();
...
test.only('if Sigaa http cache page', async () => {
  const http = createHTTPInstance();
  await http.http.get('/sigaa/public/home.jsf'); // request to get cookies
...
test.only('if Sigaa http return page same request', async () => {
  const http = createHTTPInstance();
